...
;	C:\ti\ccsv6\tools\compiler\ti-cgt-arm_5.2.4\bin\armacpia.exe -@C:\\Users\\VALVAN~1\\AppData\\Local\\Temp\\0340012 
	.sect	".text"
	.clink
...
$C$DW$1	.dwtag  DW_TAG_subprogram, DW_AT_name("SysTick_Init")
	...
	.dwpsn	file "../SysTick.c",line 44,column 24,is_stmt,address SysTick_Init,isa 1
...
;----------------------------------------------------------------------
;  45 | NVIC_ST_CTRL_R = 0;                   // disable SysTick during setup  
;----------------------------------------------------------------------
        LDR       A2, $C$CON1           ; [DPU_3_PIPE] |45| 
        MOVS      A1, #0                ; [DPU_3_PIPE] |45| 
        STR       A1, [A2, #0]          ; [DPU_3_PIPE] |45| 
	.dwpsn	file "../SysTick.c",line 46,column 3,is_stmt,isa 1
;----------------------------------------------------------------------
;  46 | NVIC_ST_RELOAD_R = NVIC_ST_RELOAD_M;  // maximum reload value          
;----------------------------------------------------------------------
        LDR       A2, $C$CON2           ; [DPU_3_PIPE] |46| 
        MVN       A1, #-16777216        ; [DPU_3_PIPE] |46| 
        STR       A1, [A2, #0]          ; [DPU_3_PIPE] |46| 
	.dwpsn	file "../SysTick.c",line 47,column 3,is_stmt,isa 1
;----------------------------------------------------------------------
;  47 | NVIC_ST_CURRENT_R = 0;                // any write to current clears it
;  48 |                                       // enable SysTick with core clock
;----------------------------------------------------------------------
        LDR       A2, $C$CON3           ; [DPU_3_PIPE] |47| 
        MOVS      A1, #0                ; [DPU_3_PIPE] |47| 
        STR       A1, [A2, #0]          ; [DPU_3_PIPE] |47| 
	.dwpsn	file "../SysTick.c",line 49,column 3,is_stmt,isa 1
;----------------------------------------------------------------------
;  49 | NVIC_ST_CTRL_R = NVIC_ST_CTRL_ENABLE+NVIC_ST_CTRL_CLK_SRC;             
;----------------------------------------------------------------------
        LDR       A2, $C$CON1           ; [DPU_3_PIPE] |49| 
        MOVS      A1, #5                ; [DPU_3_PIPE] |49| 
        STR       A1, [A2, #0]          ; [DPU_3_PIPE] |49| 
	.dwpsn	file "../SysTick.c",line 50,column 1,is_stmt,isa 1
...
$C$DW$3	.dwtag  DW_TAG_subprogram, DW_AT_name("SysTick_Wait")
	...
	.dwpsn	file "../SysTick.c",line 53,column 34,is_stmt,address SysTick_Wait,isa 1

	.dwfde $C$DW$CIE, SysTick_Wait
$C$DW$4	.dwtag  DW_TAG_formal_parameter, DW_AT_name("delay")
...
$C$DW$7	.dwtag  DW_TAG_variable, DW_AT_name("startTime")
...
;----------------------------------------------------------------------
;  53 | void SysTick_Wait(uint32_t delay){                                     
;  54 | volatile uint32_t elapsedTime;                                         
;----------------------------------------------------------------------
        STR       A1, [SP, #0]          ; [DPU_3_PIPE] |53| 
	.dwpsn	file "../SysTick.c",line 55,column 22,is_stmt,isa 1
;----------------------------------------------------------------------
;  55 | uint32_t startTime = NVIC_ST_CURRENT_R;                                
;  56 | do{                                                                    
;----------------------------------------------------------------------
...
;* --------------------------------------------------------------------------*
||$C$L1||:    
	.dwpsn	file "../SysTick.c",line 57,column 5,is_stmt,isa 1
;----------------------------------------------------------------------
;  57 | elapsedTime = (startTime-NVIC_ST_CURRENT_R)&0x00FFFFFF;                
;----------------------------------------------------------------------
        LDR       A2, $C$CON3           ; [DPU_3_PIPE] |57| 
        LDR       A1, [SP, #8]          ; [DPU_3_PIPE] |57| 
        LDR       A2, [A2, #0]          ; [DPU_3_PIPE] |57| 
        SUBS      A1, A1, A2            ; [DPU_3_PIPE] |57| 
        BIC       A1, A1, #-16777216    ; [DPU_3_PIPE] |57| 
        STR       A1, [SP, #4]          ; [DPU_3_PIPE] |57| 
	.dwpsn	file "../SysTick.c",line 59,column 9,is_stmt,isa 1
;----------------------------------------------------------------------
;  59 | while(elapsedTime <= delay);                                           
;----------------------------------------------------------------------
        LDR       A1, [SP, #0]          ; [DPU_3_PIPE] |59| 
        LDR       A2, [SP, #4]          ; [DPU_3_PIPE] |59| 
        CMP       A1, A2                ; [DPU_3_PIPE] |59| 
        BCS       ||$C$L1||             ; [DPU_3_PIPE] |59| 
        ; BRANCHCC OCCURS {||$C$L1||}    ; [] |59| 
;* --------------------------------------------------------------------------*
	.dwpsn	file "../SysTick.c",line 60,column 1,is_stmt,isa 1
...
$C$DW$8	.dwtag  DW_TAG_TI_branch
	...
	.dwattr $C$DW$3, DW_AT_TI_end_line(0x3c)
...
$C$DW$9	.dwtag  DW_TAG_subprogram, DW_AT_name("SysTick_Wait10ms")
	...
	.dwpsn	file "../SysTick.c",line 63,column 38,is_stmt,address SysTick_Wait10ms,isa 1

	.dwfde $C$DW$CIE, SysTick_Wait10ms
$C$DW$10	.dwtag  DW_TAG_formal_parameter, DW_AT_name("delay")
...
$C$DW$11	.dwtag  DW_TAG_variable, DW_AT_name("delay")
...
;----------------------------------------------------------------------
;  63 | void SysTick_Wait10ms(uint32_t delay){                                 
;  64 | uint32_t i;                                                            
;----------------------------------------------------------------------
        STR       A1, [SP, #0]          ; [DPU_3_PIPE] |63| 
	.dwpsn	file "../SysTick.c",line 65,column 7,is_stmt,isa 1
;----------------------------------------------------------------------
;  65 | for(i=0; i<delay; i++){                                                
;----------------------------------------------------------------------
        MOVS      A1, #0                ; [DPU_3_PIPE] |65| 
        STR       A1, [SP, #4]          ; [DPU_3_PIPE] |65| 
	.dwpsn	file "../SysTick.c",line 65,column 12,is_stmt,isa 1
        LDR       A2, [SP, #4]          ; [DPU_3_PIPE] |65| 
        LDR       A1, [SP, #0]          ; [DPU_3_PIPE] |65| 
        CMP       A1, A2                ; [DPU_3_PIPE] |65| 
        BLS       ||$C$L3||             ; [DPU_3_PIPE] |65| 
...
;* --------------------------------------------------------------------------*
||$C$L2||:    
	.dwpsn	file "../SysTick.c",line 66,column 5,is_stmt,isa 1
;----------------------------------------------------------------------
;  66 | SysTick_Wait(160000);  // wait 10ms (assumes 16 MHz clock)             
;----------------------------------------------------------------------
...
$C$DW$13	.dwtag  DW_TAG_TI_branch
	...
	.dwattr $C$DW$13, DW_AT_TI_call
        BL        SysTick_Wait          ; [DPU_3_PIPE] |66| 
        ; CALL OCCURS {SysTick_Wait }    ; [] |66| 
	.dwpsn	file "../SysTick.c",line 65,column 21,is_stmt,isa 1
        LDR       A1, [SP, #4]          ; [DPU_3_PIPE] |65| 
	...
	.dwpsn	file "../SysTick.c",line 65,column 12,is_stmt,isa 1
        ...
        LDR       A2, [SP, #4]          ; [DPU_3_PIPE] |65| 
        CMP       A1, A2                ; [DPU_3_PIPE] |65| 
        BHI       ||$C$L2||             ; [DPU_3_PIPE] |65| 
        ; BRANCHCC OCCURS {||$C$L2||}    ; [] |65| 
;* --------------------------------------------------------------------------*
	.dwpsn	file "../SysTick.c",line 68,column 1,is_stmt,isa 1
;* --------------------------------------------------------------------------*
||$C$L3||:    
$C$DW$14	.dwtag  DW_TAG_TI_branch
	...
	.dwattr $C$DW$9, DW_AT_TI_end_line(0x44)
...
$C$DW$T$54	.dwtag  DW_TAG_typedef, DW_AT_name("__builtin_va_list")
	...
	.dwattr $C$DW$T$54, DW_AT_decl_file("../SysTick.c")
